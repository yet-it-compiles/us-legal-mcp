...
console.error(`   CourtListener: ${apiKeys.courtListener ? "✅ Set" : "❌ Missing"}`);
console.error(`   Regulations.gov: ${apiKeys.regulationsGov ? "✅ Set" : "❌ Missing"}`);
    ...
    try {
        switch (name) {
            case "search_congress_bills": {
                const { query, congress, limit } = args;
                try {
            ...
            case "search_federal_register": {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `**Federal Register Search Results for "${query}"**\n\nFound ${documents.length} result(s)\n\n` +
                                documents
            ...
            case "search_all_legal": {
                ...
                const [bills, regulations, opinions] = await Promise.all([
                    usLegalAPI.congress.searchBills(query, undefined, Math.ceil((limit || 10) / 3)),
                    usLegalAPI.federalRegister.searchDocuments(query, Math.ceil((limit || 10) / 3)),
                    usLegalAPI.courtListener.searchOpinions(query, undefined, Math.ceil((limit || 10) / 3)),
                ]);
                return {
                    content: [
                        {
                            type: "text",
                            text: `**Comprehensive US Legal Search Results for "${query}"**\n\n` +
                                `- Bills: ${bills.length}\n` +
                                ...
                                (bills.length > 0 ?
                                    `**Congress Bills (${bills.length}):**\n` +
                                        bills
                                            .slice(0, 3)
                                            .map((bill, index) => `${index + 1}. **${bill.title}** (Bill)\n   ${bill.type} ${bill.number}\n   ${bill.url}\n`)
                                            .join("\n") +
                                        "\n\n"
                                    : "") +
                                (regulations.length > 0 ?
                                    `**Federal Register (${regulations.length}):**\n` +
                                        regulations
                                            .slice(0, 3)
                                            .map((doc, index) => `${index + 1}. **${doc.title}** (Regulation)\n   ${doc.document_number}\n   ${doc.html_url}\n`)
                                            .join("\n") +
                                        "\n\n"
                                    : "") +
                                (opinions.length > 0 ?
                                    `**Court Opinions (${opinions.length}):**\n` +
                                        opinions
                                            .slice(0, 3)
                                            .map((opinion, index) => `${index + 1}. **${opinion.case_name}** (Court Case)\n   ${opinion.court} - ${opinion.date_filed}\n   ${opinion.url}\n`)
                                            .join("\n")
                                    : "") +
                                ((bills.length === 0 &&
                                    regulations.length === 0 &&
            ...
            case "get_recent_regulations": {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `**Recent Federal Register Documents**\n\nFound ${documents.length} result(s)\n\n` +
                                documents
                                    .map((doc, index) => `${index + 1}. **${doc.title}**\n   ${doc.document_number} - ${doc.agency_names?.[0] || "Unknown agency"}\n   ${doc.html_url}\n`)
                                    .join("\n"),
                        },
            ...
            case "search_court_opinions": {
                const { query, court, limit } = args;
                try {
                    const opinions = await usLegalAPI.courtListener.searchOpinions(query, court, limit);
                    ...
                    const fetchedOpinions = await Promise.all(topOpinionsToFetch.map((op) => usLegalAPI.courtListener.getOpinion(op.id).catch(() => null)));
                    // Create a map of fetched opinions
                    const fetchedMap = new Map(fetchedOpinions
                        .filter((op) => op !== null)
                        .map((op) => [op.id, op]));
            ...
            case "get_recent_court_opinions": {
                const { court, limit } = args;
                try {
                    const opinions = await usLegalAPI.courtListener.getRecentOpinions(court, limit);
                    if (opinions.length === 0) {
                        return {
                            content: [
                                {
            ...
            case "get_congress_committees": {
                ...
                const committees = await usLegalAPI.congress.getCommittees(congress, chamber);
                return {
                    content: [
                        {
                            type: "text",
                            text: `**Congress Committees**\n\nFound ${committees.length} result(s)\n\n` +
                                committees
                                    .map((committee, index) => `${index + 1}. **${committee.name}**\n   ${committee.chamber || "N/A"} - ${committee.committeeType || "N/A"}\n   ${committee.url}\n`)
                                    .join("\n"),
                        },
                    ],
                };
        ...
        tools: [
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        query: {
                            type: "string",
                            description: "Search query for bills (e.g., 'immigration', 'healthcare', 'infrastructure')",
                        },
                        congress: {
                            type: "number",
            ...
            },
            {
                ...
                description: "Search for documents in the Federal Register (regulations, executive orders, etc.)",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
                            type: "string",
                            description: "Search query for regulations (e.g., 'environmental', 'healthcare', 'immigration')",
                        },
                        limit: {
                            type: "number",
                            description: "Number of results to return (max 50)",
            ...
            },
            {
                ...
                description: "Comprehensive search across all US legal sources (Congress, Federal Register, Court Opinions)",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
            ...
            },
            {
                ...
                description: "Get the most recently published Federal Register documents",
                inputSchema: {
                    type: "object",
                    properties: {
                        limit: {
            ...
            },
            {
                ...
                description: "Search for court opinions and decisions from CourtListener (federal and state courts)",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
                            type: "string",
                            description: "Search query for court opinions (e.g., 'immigration', 'copyright', 'constitutional')",
                        },
                        court: {
                            type: "string",
                            description: "Optional court filter (e.g., 'scotus', 'ca1', 'ca2')",
                        },
                        limit: {
                            type: "number",
                            description: "Number of results to return (max 50)",
            ...
            },
            {
                ...
                description: "Get the most recently published court opinions from CourtListener",
                inputSchema: {
                    type: "object",
                    properties: {
                        court: {
